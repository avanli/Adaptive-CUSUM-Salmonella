...
get_ARL_CUSUMAdaptive<-function(beta,mdl.prm,h_s, time_inj, n_MC,flag_trendShift,sm_prm,thres.resid){
...
while (j <=n_MC){
  ...
  #muhat=0.1*lambda0      # EWMA smoother with head start 
  delta_hat=delta0
  shift_hat=0
  muhatVec=0
  yhatINGARCH=lambda0
  while (S[i]<h_s| i<=time_inj){
    ...
    shift_hat[i]=muhat*(1-b1-b2)                #V1
    #shift_hat[i]=muhat                          #V2

# Adaptive CUSUM for step shift detection- Double exponential smoother

get_ARL_CUSUMAdaptiveDES<-function(beta,mdl.prm,h_s, time_inj, n_MC,flag_trendShift,sm_prm,thres.resid,sm_prm2){
...
while (j <=n_MC){
  ...
  muhat=0  # initialize EWMA smoother   # V2
  # muhat=lambda0 # initialize EWMA smoother  #V1
  delta_hat=delta0
  SlopePrev=0
  LevelPrev=lambda0
  yhatINGARCH=lambda0
  ...
  while (S[i]<h_s| i<=time_inj){
    ...
    resid=y[i]-yhatINGARCH[i]-muhat   # V1
    #resid=y[i]-yhatINGARCH[i]-muhat*(1-b1-b2)   # V2
    ...
    SlopeNew=SlopePrev+ sm_prm2*modified.resid
    
    muhat=LevelNew+SlopeNew

    yhat[i]=muhat+yhatINGARCH[i]     # V1
    #yhat[i]=muhat*(1-b1-b2)+yhatINGARCH[i]     # V2
    
    
    shift_hat[i]=muhat# V1
    # shift_hat[i]=muhat*(1-b1-b2)# V2
    
    k=i-1
 
    if (lambda1[i]>0 & lambda0[i]>0 & yhat[i]>0){
      weight=shift_hat[i]  # V1 & V2
...
result=list(ARL=ARL,seRL=seRL,tau_est=tau_est,bias=bias,
            mse=mse,SLastRun=S,YLastRun=y,lamOut=lambda1,yhat=yhat,delta_hat=delta_hat,shift_hat=shift_hat)
return(result)

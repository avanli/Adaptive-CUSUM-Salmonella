...
# used for chart tuning
path="C:\\Users\\ardav\\Dropbox\\Publications\\MonitorSeasonalPoisson\\R\\"
source(paste(path,"get_ARL_CUSUM.R",sep=""))
source(paste(path,"get_ARL_CUSUMAdaptive.R",sep=""))
source(paste(path,"get_ARL_CUSUMAdaptiveDES.R",sep=""))
...
#windows(7,10)
png("Plot3.png", width = 7, height = 10, units = 'in', res = 600)
shiftScale=2
...
#legend("topleft",legend=c('data','forecast','shift estimate'),
#       col=c("black","blue","orange"),
#       lty=c(1,1,1),lwd=c(1,2,2),cex=0.95)
...
abline(h=1)
#legend("topleft",legend=c('monitoring began','alarm'),
#       col=c("blue","green"),pch=c(2,2),cex=0.95)
# panels 3 and 4 - EWMA
shiftScale=1
...
dev.off()
#### find alarm thresholds using parameters estimated from N data####
if(flag_ChartTuning){
n_MC=1000
...
h_s=.93
set.seed(98765)
res=get_ARL_CUSUMAdaptive(beta,mdl.prmEst_NoTrend, h_s, time_inj, 
...
h_s=5.4
set.seed(98765)
res=get_ARL_CUSUMAdaptiveDES(beta,mdl.prmEst_NoTrend, h_s, time_inj, 
...
# for reproducibility
set.seed(98765)
res=get_ARL_CUSUM(beta,beta_c,mdl.prmEst_NoTrend, h_s, time_inj, 
